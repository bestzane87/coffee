...
Page({
  ...
  onLoad: function (options) {
    ...
    if (!userMsg) {
      let pageUrl = `/pages/activityDetail/activityDetail?activity_id=${sceneArr[0]}${sceneArr[1] ? '&from_user_id=' + sceneArr[1] : ''}&share=true`
    ...
    this.setData({
      options: {
        from_user_id: sceneArr[1],
        activity_id: sceneArr[0]
      },
      fromUserId: sceneArr[1] || '',
      share: true
    })

    wx.setStorageSync('activityId', sceneArr[0])

    if (sceneArr[1]) {
      wx.setStorageSync('fromUserId', sceneArr[1])
    }
  },
